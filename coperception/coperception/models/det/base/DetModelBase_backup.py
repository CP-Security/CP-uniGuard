...
class DetModelBase(nn.Module):
    ...
    def build_local_communication_matrix(feature_list: list):
        ...
        valid_features = [feat for feat in feature_list if feat.numel() > 0]
        
        if not valid_features:
            raise ValueError("All feature tensors are empty!")
        
        # Ensure all tensors have the same dimensions except for the concatenation dimension
        shapes = [feat.shape for feat in valid_features]
        if len(set(shapes)) > 1:
            print(f"Warning: Feature shapes are inconsistent: {shapes}")
            # Try to make shapes consistent by padding or reshaping
            target_shape = shapes[0]
            consistent_features = []
            for feat in valid_features:
                if feat.shape != target_shape:
                    print(f"Reshaping feature from {feat.shape} to {target_shape}")
                    # Simple fix: repeat the tensor if it's smaller
                    if feat.shape[0] < target_shape[0]:
                        repeat_times = target_shape[0] // feat.shape[0]
                        feat = feat.repeat(repeat_times, 1, 1, 1, 1)
                consistent_features.append(feat)
            valid_features = consistent_features
