...
from adaptive_threshold import OnlineAdaptiveThreshold
...
def main(args):
    ...
    if args.log:
        ...
        log_file_name = os.path.join(model_save_path, "log_epoch{}_scene{}_ego{}_{}attackers_{}_{}_initial_threshold_{}.txt".format(checkpoint["epoch"], args.scene_id, args.ego_agent, args.number_of_attackers, args.robosac, time_str(), args.initial_threshold))
    ...
    # Initialize adaptive threshold for PASAC algorithm (persistent across frames)
    adaptive_threshold = None
    if args.robosac == 'pasac_mAP':
        adaptive_threshold = OnlineAdaptiveThreshold(
            alpha=args.adaptive_alpha,  # Smoothing factor from args
            gamma=args.adaptive_gamma,  # Depth adjustment parameter from args
            window_size=args.adaptive_window_size,  # Window size from args
            min_window_size=args.adaptive_min_window_size,  # Minimum samples from args
            initial_threshold=args.box_matching_thresh,
            use_median=True,  # Use median for robustness
            min_neg_per_frame=1,  # Always collect at least one negative sample per frame
            decay_factor=0.98  # Gradual decay when no samples
        )
        print_and_write_log("Initialized dual-buffer adaptive threshold with parameters: alpha={:.3f}, gamma={:.3f}, window_size={}, min_window_size={}, initial_threshold={:.4f}".format(
            args.adaptive_alpha, args.adaptive_gamma, args.adaptive_window_size, args.adaptive_min_window_size, args.box_matching_thresh))
    ...
    assert args.robosac in ["upperbound", "lowerbound", "no_defense", "robosac_validation", "robosac_mAP", "adaptive", "fix_attackers", "performance_eval", "probing", "pasac_mAP", "linear_mAP"]
    ...
    for cnt, sample in enumerate(tqdm(validation_data_loader)):
        ...
        if args.robosac == 'upperbound':
            ...
            if args.robosac == "probing" :
            ...
            elif args.robosac == 'pasac_mAP':
                # PASAC algorithm adapted for 3D object detection
                # Progressive Agent Selection using Adversarial Consensus
                # Now with Online Adaptive Threshold via Temporal Sliding-Window Quantile
                ...
                print_and_write_log("\nPASAC Algorithm with Adaptive Threshold: Frame {}".format(idx))
                print_and_write_log("Available agents (excluding ego {}): {}".format(ego_idx, all_agent_list))

                # Initialize adaptive threshold if not already done
                ...
                # First calculate ego only prediction as reference
                if args.use_history_frame == True:
                    # use history frame to save one forward pass
                    if int(idx) == 0:
                        # first frame, use current ego only result as reference result
                ...
                def calculate_consensus_score(agent_list):
                    """Calculate consensus score (Jaccard Index) for given agent list"""
                    if not agent_list:
                        return 0.0
                    
                    # Always include ego agent in collaboration when calculating consensus score
                    collab_agents_with_ego = [ego_idx] + agent_list
                    
                    data['pert'] = pert.to(device)
                    data['no_fuse'] = False
                    data['collab_agent_list'] = collab_agents_with_ego
                    # Use the original attacker_list, not the dynamically calculated one
                    data['attacker_list'] = attacker_list  # This is the original attacker list from the attack generation
                    
                    loss, cls_loss, loc_loss, result_fused = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    
                    # Calculate Jaccard index between ego-only and ego+collab fused result
                    jac_index = get_jaccard_index(config, num_agent_list, padded_voxel_points, reg_target, anchors_map, gt_max_iou, result_reference, result_fused)
                    
                    print_and_write_log("Agent list {} (with ego {}) vs attackers {}: Jaccard Coefficient = {:.4f}".format(agent_list, ego_idx, attacker_list, jac_index))
                    return jac_index

                def pasac_recursive(agent_list, depth=0):
                    """Recursively execute PASAC algorithm for 3D object detection with dual-buffer adaptive threshold"""
                    indent = "  " * depth
                    print_and_write_log("{}PASAC recursive call: agents={}".format(indent, agent_list))
                    
                    if len(agent_list) <= 1:
                        if len(agent_list) == 0:
                            return []
                        # If only one agent remains, directly calculate its consensus score
                        consensus_score = calculate_consensus_score(agent_list)
                        
                        # Get current adaptive threshold with depth adjustment
                        current_threshold = adaptive_threshold.get_threshold_with_depth_adjustment(depth)
                        
                        # Determine if this is benign based on threshold
                        is_benign = consensus_score >= current_threshold
                        result = agent_list if is_benign else []
                        
                        # Record individual agent test result
                        agent_id = agent_list[0]
                        all_tested_agents[agent_id] = (consensus_score, is_benign)
                        
                        # Add score to appropriate buffer based on benign/non-benign status
                        adaptive_threshold.add_score(consensus_score, is_benign)
                        
                        print_and_write_log("{}Base case: agent {} -> consensus={:.4f}, threshold={:.4f}, result={}".format(
                            indent, agent_list[0], consensus_score, current_threshold, result))
                        return result
                    ...
                    print_and_write_log("{}Splitting {} agents: G1={}, G2={}".format(indent, len(agent_list), G1, G2))
                    
                    # Calculate consensus scores for both groups
                    consensus_G1 = calculate_consensus_score(G1)
                    consensus_G2 = calculate_consensus_score(G2)
                    ...
                    # Check G1 against adaptive threshold and add to appropriate buffer
                    is_G1_benign = consensus_G1 >= current_threshold
                    adaptive_threshold.add_score(consensus_G1, is_G1_benign)
                    
                    # Always recurse on G1, but only collect agents if the group is benign
                    if is_G1_benign:
                        print_and_write_log("{}G1 passed adaptive threshold ({:.4f} >= {:.4f}), recursing and collecting agents...".format(
                            indent, consensus_G1, current_threshold))
                        benign_agents.extend(pasac_recursive(G1, depth + 1))
                    else:
                        print_and_write_log("{}G1 failed adaptive threshold ({:.4f} < {:.4f}), still recursing to identify individual agents...".format(
                            indent, consensus_G1, current_threshold))
                        # Still recurse, but don't add results to benign agents list
                        benign_agents.extend(pasac_recursive(G1, depth + 1))
                    
                    # Check G2 against the same threshold
                    is_G2_benign = consensus_G2 >= current_threshold
                    adaptive_threshold.add_score(consensus_G2, is_G2_benign)
                    
                    # Always recurse on G2, but only collect agents if the group is benign
                    if is_G2_benign:
                        print_and_write_log("{}G2 passed adaptive threshold ({:.4f} >= {:.4f}), recursing and collecting agents...".format(
                            indent, consensus_G2, current_threshold))
                        benign_agents.extend(pasac_recursive(G2, depth + 1))
                    else:
                        print_and_write_log("{}G2 failed adaptive threshold ({:.4f} < {:.4f}), still recursing to identify individual agents...".format(
                            indent, consensus_G2, current_threshold))
                        # Still recurse, but don't add results to benign agents list
                        benign_agents.extend(pasac_recursive(G2, depth + 1))
                    
                    print_and_write_log("{}Returning benign agents: {}".format(indent, benign_agents))
                    return benign_agents

                # Display current adaptive threshold status
                buffer_stats = adaptive_threshold.get_buffer_stats()
                print_and_write_log("Current adaptive threshold stats: {}".format(buffer_stats))
                
                # 添加跟踪变量记录所有测试过的代理及其状态
                all_tested_agents = {}  # 记录每个代理的测试结果 {agent_id: (score, is_benign)}
                
                # Execute PASAC algorithm with adaptive threshold
                print_and_write_log("Starting PASAC algorithm with adaptive threshold: {:.4f}".format(adaptive_threshold.get_current_threshold()))
                benign_agents = pasac_recursive(all_agent_list)
                
                # 处理并分析所有测试结果
                all_agents_status = {}
                for agent_id, (score, is_benign) in all_tested_agents.items():
                    all_agents_status[agent_id] = "benign" if is_benign else "potential attacker"
                
                print_and_write_log("Complete agent test results:")
                for agent_id in sorted(all_agents_status.keys()):
                    status = all_agents_status[agent_id]
                    score = all_tested_agents[agent_id][0] if agent_id in all_tested_agents else "not individually tested"
                    print_and_write_log(f"  Agent {agent_id}: {status}, score: {score}")
                
                # Process all collected scores at the end of the frame
                adaptive_threshold.end_frame_processing()
                
                # Display final threshold status
                final_buffer_stats = adaptive_threshold.get_buffer_stats()
                print_and_write_log("Final adaptive threshold stats: {}".format(final_buffer_stats))
                
                # If no benign agents were found for too many consecutive frames, consider resetting the threshold
                if not benign_agents and buffer_stats['consecutive_empty_frames'] >= 3:
                    print_and_write_log("No benign agents found for {} consecutive frames. Considering threshold reset.".format(
                        buffer_stats['consecutive_empty_frames']))
                    # Only reset if positive buffer is much larger than negative or vice versa (imbalanced)
                    if (buffer_stats['positive_buffer_size'] > 3 * buffer_stats['negative_buffer_size'] or
                        buffer_stats['negative_buffer_size'] > 3 * buffer_stats['positive_buffer_size']):
                        adaptive_threshold.reset_buffers()
                        print_and_write_log("Buffers have become imbalanced. Performing full reset.")
                
                # Set final prediction based on results
                if not benign_agents:
                    print_and_write_log('PASAC: No benign collaborators found, using ego only.')
                    data['pert'] = None
                    data['no_fuse'] = True
                    data['collab_agent_list'] = None
                    data['attacker_list'] = attacker_list  # Use original attacker list even in failure case
                    _, _, _, result_self_only = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    result = result_self_only
                    steps[frame_seq-1] = len(all_agent_list)  # Record number of evaluations
                    ego_steps[frame_seq-1] = 1
                    fail += 1
                else:
                    print_and_write_log('PASAC: Found {} benign collaborators: {}'.format(len(benign_agents), benign_agents))
                    # 在日志中单独输出所有确认的攻击者
                    all_potential_attackers = [i for i in all_agent_list if i in all_tested_agents and not all_tested_agents[i][1]]
                    print_and_write_log('PASAC: Identified {} potential attackers based on individual testing: {}'.format(
                        len(all_potential_attackers), all_potential_attackers))
                    
                    # 检查我们是否成功识别了所有真正的攻击者
                    correctly_identified_attackers = [i for i in attacker_list if i in all_potential_attackers]
                    if len(correctly_identified_attackers) == len(attacker_list):
                        print_and_write_log('PASAC: Successfully identified all true attackers!')
                    else:
                        print_and_write_log('PASAC: Missed some attackers. Found {}/{} attackers'.format(
                            len(correctly_identified_attackers), len(attacker_list)))
                    
                    sus_agent_list = [i for i in all_agent_list if i not in benign_agents]
                    print_and_write_log('PASAC: Potential attackers among non-collaborators: {}'.format(sus_agent_list))
                    print_and_write_log('PASAC: Actual attackers (from attack generation): {}'.format(attacker_list))
                    
                    # Always include ego agent in final collaboration
                    final_collab_agents = [ego_idx] + benign_agents
                    
                    data['pert'] = pert.to(device)
                    data['no_fuse'] = False
                    data['collab_agent_list'] = final_collab_agents
                    # Use the original attacker_list for final prediction
                    data['attacker_list'] = attacker_list
                    
                    # Final prediction with ego + benign agents
                    loss, cls_loss, loc_loss, result = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    
                    print_and_write_log('PASAC: Final collaboration with agents: {}'.format(final_collab_agents))
                    
                    # Estimate steps based on tree traversal (log-based approximation)
                    total_evaluations = 2 * len(all_agent_list) - 1  # Binary tree property
                    steps[frame_seq-1] = total_evaluations
                    succ += 1
                ...
                if args.visualization:
                    # visualize consensus result
                    visualize(config, filename0, save_fig_path, fafmodule, data, num_agent_list, padded_voxel_points, gt_max_iou, vis_tag='pasac_adaptive_consensus')

                det_results_local, annotations_local = local_eval(num_agent, padded_voxel_points, reg_target, anchors_map, gt_max_iou, result, config, det_results_local, annotations_local)

            elif args.robosac == 'linear_mAP':
                # Linear Agent Selection for Collaborative Perception
                # Individually test each agent with ego in a linear fashion
                ...
                print_and_write_log("\nLinear Agent Selection with Adaptive Threshold: Frame {}".format(idx))
                print_and_write_log("Available agents (excluding ego {}): {}".format(ego_idx, all_agent_list))
                ...
                if adaptive_threshold is None:
                    adaptive_threshold = OnlineAdaptiveThreshold(
                        ...
                        gamma=args.adaptive_gamma,
                        window_size=args.adaptive_window_size,
                        min_window_size=args.adaptive_min_window_size,
                        initial_threshold=args.initial_threshold,
                        use_median=True,
                        min_neg_per_frame=1,
                        decay_factor=0.98
                    ...
                    print_and_write_log("Initialized dual-buffer adaptive threshold with parameters: alpha={:.3f}, gamma={:.3f}, window_size={}, min_window_size={}, initial_threshold={:.4f}".format(
                        args.adaptive_alpha, args.adaptive_gamma, args.adaptive_window_size, args.adaptive_min_window_size, args.initial_threshold))
                
                # Get ego-only prediction as reference
                if args.use_history_frame == True:
                    # Use history frame to save one forward pass
                    if int(idx) == 0:
                        # First frame, use current ego only result as reference result
                        ...
                        _, _, _, result_reference = fafmodule.predict_all(data, 1, num_agent=num_agent)
                        ego_steps[frame_seq-1] = 1
                    # If not first frame, keep no-op since we use history frame
                else:
                    # If not use history frame, use current frame as reference
                    print_and_write_log("Calculating ego only result...")
                    data['pert'] = None
                    data['collab_agent_list'] = None
                    data['no_fuse'] = True
                    _, _, _, result_reference = fafmodule.predict_all(data, 1, num_agent=num_agent)
                ...
                # Linear agent testing
                benign_agents = []
                agent_test_results = {}  # {agent_id: (score, is_benign)}
                
                # Get base threshold (no depth adjustment needed for linear approach)
                current_threshold = adaptive_threshold.get_threshold_with_depth_adjustment(depth=0)
                print_and_write_log("Starting linear agent testing with threshold: {:.4f}".format(current_threshold))
                
                # Test each agent individually
                for agent_i in all_agent_list:
                    # Construct "Ego + agent_i" collaboration
                    single_agent_list = [agent_i]
                    collab_agents_with_ego = [ego_idx, agent_i]
                    
                    data['pert'] = pert.to(device)
                    data['no_fuse'] = False  
                    data['collab_agent_list'] = collab_agents_with_ego
                    data['attacker_list'] = attacker_list  # Original attacker list
                    
                    # Execute fusion prediction
                    _, _, _, result_fused_i = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    
                    # Calculate consensus score (Jaccard Index)
                    score_i = get_jaccard_index(config, num_agent_list, padded_voxel_points, 
                                              reg_target, anchors_map, gt_max_iou,
                                              result_reference, result_fused_i)
                    
                    # Compare with threshold
                    is_benign = score_i >= current_threshold
                    
                    # Add score to adaptive threshold buffer
                    adaptive_threshold.add_score(score_i, is_benign)
                    
                    # Record test result
                    agent_test_results[agent_i] = (score_i, is_benign)
                    
                    print_and_write_log("Agent {}: consensus score={:.4f}, threshold={:.4f}, status={}".format(
                        agent_i, score_i, current_threshold, "benign" if is_benign else "potential attacker"))
                    
                    # If benign, add to benign_agents list
                    if is_benign:
                        benign_agents.append(agent_i)
                    
                    # Optional visualization for each agent test
                    if args.visualization:
                        visualize(config, filename0, save_fig_path, fafmodule, data, num_agent_list,
                                 padded_voxel_points, gt_max_iou, vis_tag=f'linear_agent_{agent_i}')
                ...
                # Print summary of all agent tests
                print_and_write_log("Complete agent test results:")
                for agent_id in sorted(agent_test_results.keys()):
                    score, is_benign = agent_test_results[agent_id]
                    status = "benign" if is_benign else "potential attacker"
                    print_and_write_log(f"  Agent {agent_id}: {status}, score: {score:.4f}")
                
                # Check if we successfully identified attackers
                all_potential_attackers = [i for i in all_agent_list if i in agent_test_results and not agent_test_results[i][1]]
                correctly_identified_attackers = [i for i in attacker_list if i in all_potential_attackers]
                
                if len(correctly_identified_attackers) == len(attacker_list):
                    print_and_write_log('LINEAR: Successfully identified all true attackers!')
                else:
                    print_and_write_log('LINEAR: Missed some attackers. Found {}/{} attackers'.format(
                        len(correctly_identified_attackers), len(attacker_list)))
                
                # Set final prediction based on results
                if not benign_agents:
                    print_and_write_log('LINEAR: No benign collaborators found, using ego only.')
                    data['pert'] = None
                    data['no_fuse'] = True
                    data['collab_agent_list'] = None
                    data['attacker_list'] = attacker_list
                    _, _, _, result = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    steps[frame_seq-1] = len(all_agent_list) + 1  # ego-only + all individual tests
                    fail += 1
                else:
                    print_and_write_log('LINEAR: Found {} benign collaborators: {}'.format(len(benign_agents), benign_agents))
                    sus_agent_list = [i for i in all_agent_list if i not in benign_agents]
                    print_and_write_log('LINEAR: Potential attackers: {}'.format(sus_agent_list))
                    print_and_write_log('LINEAR: Actual attackers: {}'.format(attacker_list))
                    ...
                    # Final prediction with ego + all benign agents
                    _, _, _, result = fafmodule.predict_all(data, 1, num_agent=num_agent)
                    
                    print_and_write_log('LINEAR: Final collaboration with agents: {}'.format(final_collab_agents))
                    
                    # Record total evaluations: one per agent plus final fusion
                    steps[frame_seq-1] = len(all_agent_list) + 1  # All individual tests + final fusion
                    succ += 1
                
                if args.visualization:
                    # Visualize final consensus result
                    visualize(config, filename0, save_fig_path, fafmodule, data, num_agent_list, 
                             padded_voxel_points, gt_max_iou, vis_tag='linear_adaptive_consensus')
                ...
                # Update local evaluation metrics for mAP calculation
                det_results_local, annotations_local = local_eval(num_agent, padded_voxel_points, reg_target, 
                                                                anchors_map, gt_max_iou, result, config, 
                                                                det_results_local, annotations_local)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--robosac', type=str, default='', help='upperbound/lowerbound/no_defense/robosac_validation/robosac_mAP/adaptive/fix_attackers/performance_eval/probing/pasac_mAP/linear_mAP')
    ...
    # Online Adaptive Threshold parameters
    parser.add_argument('--adaptive_alpha', type=float, default=0.2, help='Smoothing factor for threshold updates (default: 0.2)')
    parser.add_argument('--adaptive_gamma', type=float, default=0.02, help='Additive compensation for recursive levels (default: 0.02)')
    parser.add_argument('--adaptive_window_size', type=int, default=30, help='Maximum size of sliding window for adaptive threshold (default: 30)')
    parser.add_argument('--adaptive_min_window_size', type=int, default=5, help='Minimum window size before computing adaptive threshold (default: 5)')
    parser.add_argument('--initial_threshold', type=float, default=0.3, help='Initial threshold for adaptive threshold (default: 0.3)')
