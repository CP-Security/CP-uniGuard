...
class DetModelBase(nn.Module):
    ...
    def agents_to_batch(self, feats):
        ...
        # Get the actual number of agents from the tensor shape
        actual_agent_num = feats.shape[1]  # Second dimension is agent dimension
        
        print(f"agents_to_batch: Input shape {feats.shape}, processing {actual_agent_num} agents")
        
        for i in range(actual_agent_num):
    ...
    def build_local_communication_matrix(feature_list: list):
        ...
        """
        # Debug: Print shapes of all tensors
        print(f"Feature list length: {len(feature_list)}")
        for i, feat in enumerate(feature_list):
            print(f"Agent {i} feature shape: {feat.shape}")
        
        # Filter out empty tensors and keep track of their indices
        valid_features = []
        valid_indices = []
        
        for i, feat in enumerate(feature_list):
            if feat.numel() > 0:  # Non-empty tensor
                valid_features.append(feat)
                valid_indices.append(i)
            else:
                print(f"Agent {i}: Skipping empty tensor")
        
        if not valid_features:
            raise ValueError("All feature tensors are empty!")
        
        # Ensure all valid tensors have the same shape
        reference_shape = valid_features[0].shape
        for i, feat in enumerate(valid_features):
            if feat.shape != reference_shape:
                print(f"Agent {valid_indices[i]}: Warning - shape mismatch. Expected {reference_shape}, got {feat.shape}")
        
        print(f"Using {len(valid_features)} valid agents: {valid_indices}")
        return torch.cat(tuple(valid_features), 1)
